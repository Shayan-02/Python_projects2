...
class cafe_management:
    ...
    def Total_Bill(self):
        ...
        if self.sub_cost != "":
            ...
            self.sub_cost.insert(
                END, int(self.items_cost.get()) + float(self.service_cost.get())
            )
        else:
            self.sub_cost.insert(
                END, int(self.items_cost.get()) + float(self.service_cost.get())
        ...
        if self.total_bill != "":
            ...
            self.total_bill.insert(
                END, float(self.sub_cost.get()) + float(self.paid_tax.get())
            )
        else:
            self.total_bill.insert(
                END, float(self.sub_cost.get()) + float(self.paid_tax.get())
    ...
    # ==== Exit button code =================
    def Quit(self):
        self.message = messagebox.askquestion(
    ...
    def __init__(self):
        ...
        self.date = Label(
            self.root, text=datetime.now(), font=("verdana", 10, "bold"), bg="white"
        )
        ...
        self.tea = Label(
            self.frame1, text="Tea", font=("verdana", 10, "bold"), bg="white"
        )
        self.tea.place(x=3, y=1)
        self.tea_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.tea_item.place(y=1, x=85)

        self.coffee = Label(
            self.frame1, text="Coffee", font=("verdana", 10, "bold"), bg="white"
        )
        self.coffee.place(x=3, y=20)
        self.coffee_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.coffee_item.place(y=20, x=85)

        self.sandwitch = Label(
            self.frame1, text="Sandwitch", font=("verdana", 10, "bold"), bg="white"
        )
        self.sandwitch.place(x=3, y=40)
        self.sandwitch_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.sandwitch_item.place(y=40, x=85)

        self.cake = Label(
            self.frame1, text="Cake", font=("verdana", 10, "bold"), bg="white"
        )
        self.cake.place(x=3, y=60)
        self.cake_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.cake_item.place(y=60, x=85)

        self.burger = Label(
            self.frame1, text="Burger", font=("verdana", 10, "bold"), bg="white"
        )
        self.burger.place(x=3, y=80)
        self.burger_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.burger_item.place(y=80, x=85)

        self.pizza = Label(
            self.frame1, text="Pizza", font=("verdana", 10, "bold"), bg="white"
        )
        self.pizza.place(x=3, y=100)
        self.pizza_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.pizza_item.place(y=100, x=85)

        self.fries = Label(
            self.frame1, text="Fries", font=("verdana", 10, "bold"), bg="white"
        )
        self.fries.place(x=3, y=120)
        self.fries_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.fries_item.place(y=120, x=85)

        self.pepsi = Label(
            self.frame1, text="Pepsi", font=("verdana", 10, "bold"), bg="white"
        )
        self.pepsi.place(x=3, y=140)
        self.pepsi_item = Entry(
            self.frame1, width=7, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        ...
        self.item_cost_lb = Label(
            self.frame2, text="Items Cost", font=("verdana", 10, "bold"), bg="white"
        )
        self.item_cost_lb.place(x=3, y=1)
        self.items_cost = Entry(
            self.frame2, width=9, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.items_cost.place(y=1, x=100)

        self.service_cost_lb = Label(
            self.frame2, text="Service Cost", font=("verdana", 10, "bold"), bg="white"
        )
        self.service_cost_lb.place(x=3, y=20)
        self.service_cost = Entry(
            self.frame2, width=9, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
        self.service_cost.place(y=20, x=100)

        self.sub_cost_lb = Label(
            self.frame2, text="Sub Cost", font=("verdana", 10, "bold"), bg="white"
        )
        ...
        self.paid_tax_lb = Label(
            self.frame2, text="Paid Tax", font=("verdana", 10, "bold"), bg="white"
        )
        ...
        self.total_bill_lb = Label(
            self.frame2, text="Total Bill", font=("verdana", 10, "bold"), bg="white"
        )
        self.total_bill_lb.place(x=3, y=100)
        self.total_bill = Entry(
            self.frame2, width=9, borderwidth=4, relief=SUNKEN, bg="#248aa2"
        )
