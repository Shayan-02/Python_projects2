...
class Window:
    ...
    def draw_widget(self):
        ...
        def bt_open_press():
            try:
                ...
                messagebox.showerror(
                    "FileNotFound", 'File "saved.txt" not found in directory'
                )

        # ======== Label ==========
        l_title = Label(
            self.root, text="ToDo List", font="ComicSansMs 18", bg=self.white
        )
        l_title.place(x=250, y=0)

        list_title = Label(
            self.root, text="Tasks:", font="ComicSansMs 14", bg=self.white
        )
        list_title.place(x=10, y=30)

        list_title = Label(
            self.root, text="New Task:", font="ComicSansMs 14", bg=self.white
        )
        ...
        e_input = Entry(
            self.root, width=55, font="ComicSansMs 12", bd=1, bg=self.input_bg
        )
