...
def delete_all():
    conf = messagebox.askquestion("delet all??", "are you sure to delete all task?")
    print(conf)
    if conf.upper() == "YES":
        global tasks
        tasks = []
...
def load_info():
    messagebox.showinfo(
        "info",
        "this is Todolist V.1.0 \n created by FAIZAR RAHMAN \n Python Project",
    )


def load_act():
    loadcon = messagebox.askquestion("Save Confirmation", "save your progress?")
...
sort_asc = tkinter.Button(
    root, text="sort (ASC)", bg="White", width=15, command=sort_asc
)
sort_asc.grid(row=4, column=0)

sort_dsc = tkinter.Button(
    root, text="sort (DSC)", bg="White", width=15, command=sort_dsc
)
...
exit_bttn = tkinter.Button(
    root, text="exit app", bg="white", width=15, command=exit_app
)
